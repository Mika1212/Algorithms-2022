...
abstract class AbstractGraphTests {
    ...
    fun additionalMinimumSpanningTree(minimumSpanningTree: Graph.() -> Graph) {
        val emptyGraph = GraphBuilder().build()
        assertTrue(emptyGraph.minimumSpanningTree().edges.isEmpty())
        val graph = GraphBuilder().apply {
            val a = addVertex("A")
        ...
        val tree2 = graph2.minimumSpanningTree()
        assertEquals(12, tree2.edges.size)
        assertEquals(12, tree2.findBridges().size)
        // Cross
        val graph3 = GraphBuilder().apply {
            val a = addVertex("A")
            val b = addVertex("B")
    ...
    fun additionalLongestSimplePath(longestSimplePath: Graph.() -> Path) {
        val emptyGraph = GraphBuilder().build()
        assertEquals(0, emptyGraph.longestSimplePath().length)
        ...
        val longestUnconnectedPath = unconnected.longestSimplePath()
        assertEquals(3, longestUnconnectedPath.length)
